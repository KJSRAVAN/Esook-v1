...
void main() {
      ...
      routes: {
        ...
        AppRoutes.adminPending: (context) {
          ...
          return AdminMagicLinkPendingScreen(
            email: args?['email'] ?? 'admin@esouq.com',
  ...
  Widget buildPendingWidget({String email = 'admin@esouq.com'}) {
    return MaterialApp(
      theme: AppTheme.lightTheme,
      routes: {
        AppRoutes.adminMagicLink: (_) => const Scaffold(body: Text('Admin Request Shell')),
      },
      home: AuthScope(
        repository: mockRepo,
  ...
  group('Admin Magic Link Flow', () {
    testWidgets('request screen validates email and navigates to pending screen', (tester) async {
      ...
      await tester.tap(find.widgetWithText(ElevatedButton, 'Send Login Link'));
      await tester.pumpAndSettle();

      expect(find.text('Please enter a valid email address'), findsOneWidget);
      expect(mockRepo.requestMagicLinkCallCount, equals(0));

      // Submit valid email
      await tester.enterText(find.byType(TextFormField), 'admin@esouq.com');
      await tester.tap(find.widgetWithText(ElevatedButton, 'Send Login Link'));
      await tester.pumpAndSettle();

      expect(mockRepo.requestMagicLinkCallCount, equals(1));
      expect(mockRepo.lastMagicLinkEmail, equals('admin@esouq.com'));
      expect(find.text('Check Your Email'), findsOneWidget);
    });

    testWidgets('pending screen renders email and throttles resend button', (tester) async {
      await tester.pumpWidget(buildPendingWidget(email: 'superadmin@esouq.com'));
      ...
      // Tap resend
      await tester.tap(find.widgetWithText(OutlinedButton, 'Resend Login Link'));
      await tester.pumpAndSettle();

      expect(mockRepo.requestMagicLinkCallCount, equals(1));
      expect(find.text('A fresh login link has been sent to your email.'), findsOneWidget);
    });

    testWidgets('verify screen redeems valid token and navigates to adminHome', (tester) async {
      ...
      await tester.pumpWidget(buildVerifyWidget(token: 'valid_secure_token_123'));
      await tester.pumpAndSettle();
    ...
    testWidgets('verify screen handles expired/invalid token with retry action', (tester) async {
      mockRepo.verifyMagicLinkResult = Result.failure(
        const UnauthorizedFailure(message: 'Magic link has expired or is invalid.'),
      );
      ...
      expect(find.text('Magic link has expired or is invalid.'), findsOneWidget);
      expect(find.widgetWithText(ElevatedButton, 'Request New Link'), findsOneWidget);

      await tester.tap(find.widgetWithText(ElevatedButton, 'Request New Link'));
      await tester.pumpAndSettle();
