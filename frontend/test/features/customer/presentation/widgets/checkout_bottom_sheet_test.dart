...
void main() {
  group('CheckoutBottomSheet', () {
    ...
    testWidgets('renders checkout summary, fulfillment options, and fields', (tester) async {
      ...
      expect(find.byKey(const Key('checkout_delivery_address_input')), findsOneWidget);
    ...
    testWidgets('validates that delivery address is required when fulfillment is Delivery', (tester) async {
      setupViewport(tester);
      await tester.pumpWidget(buildCheckoutHost());
      await tester.tap(find.text('Open Checkout'));
      await tester.pumpAndSettle();
      ...
      expect(find.text('Delivery address is required for delivery orders'), findsOneWidget);
      expect(mockOrderRepository.createOrderCallCount, equals(0));
    });

    testWidgets('allows placing order without delivery address when Pickup is selected', (tester) async {
      setupViewport(tester);
      OrderModel? placedOrder;
      await tester.pumpWidget(buildCheckoutHost(
        onOrderSuccess: (order) => placedOrder = order,
      ));
      await tester.tap(find.text('Open Checkout'));
      await tester.pumpAndSettle();
      ...
      expect(find.byKey(const Key('checkout_delivery_address_input')), findsNothing);
      ...
      expect(mockOrderRepository.lastFulfillmentParam, equals(FulfillmentType.pickup));
    ...
    testWidgets('places delivery order with address, notes, and coupon code', (tester) async {
      setupViewport(tester);
      OrderModel? placedOrder;
      await tester.pumpWidget(buildCheckoutHost(
        onOrderSuccess: (order) => placedOrder = order,
      ...
      expect(mockOrderRepository.lastFulfillmentParam, equals(FulfillmentType.delivery));
      expect(mockOrderRepository.lastDeliveryAddressParam, equals('Building 10, King Fahd Rd'));
      expect(mockOrderRepository.lastNotesParam, equals('Please call on arrival'));
      expect(mockOrderRepository.lastCouponCodeParam, equals('SAVE10'));
      expect(mockOrderRepository.lastIdempotencyKeyParam, isNotNull);
      expect(mockOrderRepository.lastIdempotencyKeyParam!.length, greaterThan(10));
      expect(placedOrder, isNotNull);
    });

    testWidgets('displays error banner when order creation fails', (tester) async {
      setupViewport(tester);
      ...
      expect(find.text('Selected store is currently not accepting orders'), findsOneWidget);
    });

    testWidgets('prevents duplicate submissions from rapid repeated taps', (tester) async {
      setupViewport(tester);
