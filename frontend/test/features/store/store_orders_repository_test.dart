...
void main() {
  group('StoreOrdersRepositoryImpl', () {
    ...
    test(
      'getStoreOrders queries /orders/store/:storeId and maps scoped orders',
      () async {
    ...
    test(
      'updateOrderStatus sends PATCH /orders/:id/status with uppercase backend status',
      () async {
        mockTransport.statusCode = 200;
        mockTransport.responseBody = jsonEncode({
          'order': {
    ...
    test('updateOrderStatus sends DELIVERED in uppercase', () async {
      mockTransport.statusCode = 200;
      mockTransport.responseBody = jsonEncode({
        'order': {
          'id': 'ord-101',
      ...
      expect(result.isSuccess, isTrue);
      expect(mockTransport.lastBody, contains('"status":"DELIVERED"'));
    });
    ...
    test('updateOrderStatus handles rejection with rejectedReason', () async {
      ...
      expect(
        ...
        contains('"rejectedReason":"Store closed for maintenance"'),
      );
    });
  });
}
